...
Use the PR diff to review `lib/live-*`, `lib/host-session.ts`, `lib/service-generator.ts`, the opt-in configuration and native hooks. Review the existing scope gate regression separately from the new conversational restriction. ClaudeLive vendors these modules with local imports. This update ports native principal activity, late worker observation and bounded handshake recovery back to ClawCode; its own hooks and host_native policy remain authoritative. The wire protocol remains LiveBridge v1; the copies are not identical. Both now distinguish publication from audio playback and suppress a second spoken answer for `input.completed`. See ClaudeLive's [upstream provenance](https://github.com/crisandrews/ClaudeLive/blob/main/src/server/bridge/README.md) for the pinned revision and remaining local differences.
...
`channels_detect` can generate both channel flags when Live is already opted in. `service_plan` preserves flags provided in `extraArgs` and now adds the opted-in ClawCode Live channel to a new install plan. An already running service is unchanged. Preserve both channel entries when regenerating the service, together with all its existing arguments:
...
- **Fresh voice status:** GPT-Live can discuss already known public context without a new leader turn. When a question needs a fresh status check or other new work, it delegates to the same leader, whose Channels input can wait behind current work. A guaranteed immediate fresh check is not implemented.
- **Native task reconciliation:** the board contains work explicitly published with `live_work` or observed through this bridge's native hooks. A verified tool request can now recover a worker whose start was missed, and main-session tool activity is projected independently of task cards. This still does not enumerate dormant historical agents. Terminal cards are not reopened by late hooks; the leader must republish verified existing/resumed work with its stable native identity.
