---
name: live
description: Set up, check, reconfigure or disable ClaudeLive conversations for this existing ClawCode agent. Use for /agent:live, "enable ClaudeLive", "connect voice to Cloudy", or "habilita ClaudeLive". Keeps the existing WhatsApp channels and agent context.
user-invocable: true
...
Call `live_setup_plan({ enabled: false, ... })` using the established workspace/launch context, then apply its reviewed command under the owner's existing authorization. This changes the saved enable flag and retains credentials, messages and tasks. Do not run install or launch commands returned for setup. Report whether a running host still needs its normal restart to stop the bridge. Remove only ClawCode's Live channel from the next launch if requested; preserve WhatsApp and all other arguments. Ending voice by itself does not stop Cloudy or its work.
